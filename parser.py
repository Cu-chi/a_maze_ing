...
class ConfigError(Exception):
    def __init__(self, line_index: int, *args: object) -> None:
        super().__init__(*args)
...
def parsed_info(file: TextIOWrapper) -> dict[str, str]:
    ...
    for line_index, line in enumerate(file, start=1):
        ...
        if line == "\n":
            raise ConfigError(line_index, "blank line in config.txt")
        if len(data) < 2:
            raise ConfigError(line_index, "the line doesn't respect key=value")
